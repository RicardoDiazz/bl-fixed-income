...
def plot_portfolio_comparison(assets, weights_eq, weights_bl):
    ...
    rects2 = ax.bar(
        x + width / 2,
        weights_bl.values * 100,
        width,
        label="Black-Litterman Ajustado",
        color="#50E3C2",
    )

    ax.set_ylabel("Asignación de Pesos (%)", fontsize=12)
    ax.set_title(
        "Comparativa de Portafolios: Mercado vs. Black-Litterman",
        fontsize=14,
        fontweight="bold",
        pad=15,
    )
...
def run_pipeline():
    ...
    pi = calculate_implied_equilibrium_returns(
        market_weights, cov_matrix, risk_aversion=risk_aversion
    )
