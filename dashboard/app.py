...
with tabs[1]:
    ...
    selected_asset = st.selectbox(
        "Selecciona el ETF de Tesoro a analizar:", assets, key="price_asset"
    )
    fig_price = px.line(
        df,
        y=selected_asset,
        title=f"Precio Histórico - {selected_asset}",
...
with tabs[2]:
    ...
    selected_ret = st.selectbox(
        "Selecciona el ETF para ver sus retornos:", assets, key="ret_asset"
    )
    fig_ret = px.line(
        df,
        y=f"{selected_ret}_ret",
        title=f"Volatilidad de Retornos Semanales - {selected_ret}",
        labels={"value": "Retorno Log", "index": "Fecha"},
    )
...
with tabs[3]:
    ...
    target_asset = st.selectbox(
        "Selecciona el Activo Objetivo:", assets, key="target_asset"
    )
    horizon = st.radio(
        "Horizonte de Predicción:", ["Q1 (1 Sem)", "Q2 (2 Sem)", "Q3 (4 Sem)"]
    )
    ...
    fig_hist = px.histogram(
        ...
        x=target_col,
        marginal="box",
        title=f"Distribución del Target: {target_col}",
        nbins=50,
        labels={target_col: "Retorno Futuro"},
    )
...
with tabs[4]:
    ...
    st.markdown(
        "Verificación visual de las variables de entrada "
        "construidas para alimentar el modelo predictivo:"
    )
